...
def train_model(model, x, edge_index, y, train_mask, val_mask, test_mask, optimizer, criterion, num_epochs, device):
    train_losses = []
    val_losses = []
    train_accuracies = []
    val_accuracies = []
    ...
    for epoch in range(num_epochs):
        ...
        train_probs = F.softmax(out[train_mask], dim=1)[:, 1]
        train_pred = (train_probs > 0.5).float().long()
        train_accuracy = (train_pred == y[train_mask]).float().mean().item()
        ...
        with torch.no_grad():
            ...
            val_accuracy = (val_pred == y[val_mask]).float().mean().item()
...
def test_model(model, x, edge_index, y, test_mask, criterion, device):
    model.eval()
    with torch.no_grad():
        out = model(x, edge_index)
        ...
        test_accuracy = (test_pred == y[test_mask]).float().mean().item()
...
def calculate_metrics(model, x, edge_index, y, train_mask, val_mask):
    model.eval()
    with torch.no_grad():
        out = model(x, edge_index)
...
def calculate_confusion_matrix(model, x, edge_index, y, train_mask, val_mask):
    model.eval()
    with torch.no_grad():
        out = model(x, edge_index)
        train_y = y[train_mask.cpu().numpy()]
...
def plot_graphs(train_losses, val_losses, train_accuracies, val_accuracies, test_aucs, fpr, tpr, test_auc, train_cm, val_cm, train_recalls, val_recalls, save_path):
    ...
    plt.plot(fpr, tpr, 'r-', label=f'GCN (AUC = {test_auc:.4f})')
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
...
def main():
    ...
    features = geochem_data.drop(columns=['Ore']).iloc[:, 2:-5].values
    ...
    num_nodes = x.size(0)
    edge_index = load_adj_matrix('geo_adj_matrix.csv', num_nodes)

    device = torch.device("cuda" if args.device == "cuda" and torch.cuda.is_available() else "cpu")
    model = GCN(input_dim=x.size(1)).to(device)
    lr = args.lr
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    criterion = torch.nn.CrossEntropyLoss()
    num_epochs = args.epochs

    result_path = os.path.join(out_dir, "GCN")
    ...
    train_losses, val_losses, train_accuracies, val_accuracies, train_aucs, val_aucs, test_aucs, train_recalls, val_recalls = train_model(
        model, x, edge_index, y, train_mask, val_mask, test_mask, optimizer, criterion, num_epochs, device
    )

    fpr, tpr, test_auc = test_model(model, x, edge_index, y, test_mask, criterion, device)

    train_kappa, train_f1, train_recall, val_kappa, val_f1, val_recall = calculate_metrics(model, x, edge_index, y, train_mask, val_mask)

    train_cm, val_cm = calculate_confusion_matrix(model, x, edge_index, y, train_mask, val_mask)
    ...
    with torch.no_grad():
        out = model(x, edge_index)
        test_probs = F.softmax(out[test_mask], dim=1)[:, 1].detach().cpu().numpy()
    ...
    roc_file_name = f"GCN_{current_time}_epochs_{num_epochs}_lr_{lr}_test_roc.csv"
